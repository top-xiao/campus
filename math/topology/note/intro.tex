...
\maketitle
\chapter*{Preface}
これは永明の位相数学ノートです，全ては定義そのものと永明本人の臆断に基づいて書かれたものです.\\
集合と位相は大学数学の基盤となっている分野です，この分野で躓いてしまうと今後大学数学のみならず，理系の純粋理論研究におけるあらゆる学問的な勉強が立ち行かなくなります.ですので，必ずこの分野を噛み砕いてついていけるように復習してください．\\
なお，このノートの読者は数学序論A,Bを履修していない方と想定して作ったものですが，論理記号と述語論理はすでに習ったこととし，もしこれらについて何もわからないのであれば全学教育の記号論理学を履修することを強くお勧めします.少し公理的集合論の予備知識があればなお良いと思います, 興味のある方は\href{https://top-xiao.github.io/blog}{私の学習ノート}に参照していただければと思います. また証明に関して任意と存在をうるさく区別して，混乱を招かないようにしています．\\
学習のアドバイスとして，定義を尊重して，定義通りに論理を展開して証明を進めるとほとんどの証明を理解できるはず.直感は大事だけれども，直感に頼りすぎると理解が危うくなり視野が狭くなりますので，イメージはあくまでイメージとして認識して，決してイメージを用いて証明をごまかしてはならない．\\
位相数学を勉強して，幾何学や多様体理論の世界に踏み出そうとする皆さんの勉強上の支障になれることをこころよりお祈り申し上げます.（笑）
\newpage
...
この部分は主にユークリッド空間の性質を一般化した位相空間の内容を議論するつもりである.位相空間の基本から始まり，まず高度に抽象的な位相空間論における用語を説明した上で，理論を展開していく．途中で距離空間論に関連する話もするつもりである．
...
次に，位相と開基との関係を示す重要なLemmaを3つ紹介する:
...
今まで扱って来た開基という概念は位相の第二性質から位相を生成する.
それが位相を作れるのは開基自身位相の第三性質を満たしているからのだ．
数学的に何かを作る時に，条件が多いほど人為的構成することが難しくなる．
今から準開基という概念を紹介して，この節を終わらせる．\\
...
上記の議論をよく見ると，$Y$順序位相空間の部分位相は常に部分集合の順序位相より強いことがわかる.
...
\begin{definition}[被覆]
	位相空間$\left( X , \mathcal{T}_{X} \right)$ の部分集合族$\mathcal{A}$ に対し, $ \cup \mathcal{A} = X$ を満たすならば, $\mathcal{A}$ を$X$の被覆\textbf{(covering)}と呼び,  さらに$\mathcal{A}$ の各元が開集合の時, $\mathcal{A}$ を$X$の開被覆\textbf{(open covering)}とする.
\end{definition}

\begin{definition}[\textcolor{red}{コンパクト性}]
	位相空間$\left( X , \mathcal{T}_{X} \right)$がコンパクト\textbf{(compact)}とは, 任意の被覆が$X$の有限被覆を部分集合に持つ.	
\end{definition}
